//! anna-v4 (piece-square only: v1 inputs, v3 output stack) trainer: threat + pawn-pair + psq inputs, pairwise FT, multilayer output.
...
use trainer::bullet_inputs::make_psq_inputs_mapper;
...
fn main() {
    let net_id: String = std::env::var("NET_ID").unwrap_or_else(|_| "anna-v4".to_string());
    ...
    let defn = ModelDefinition::build(&inputs, |builder, (((stm_psqt, ntm_psqt), output_buckets), target)| {
        // Piece-square feature transformer: an affine (weights + bias) plus the bucket-shared factoriser.
        let l0 = builder.new_affine("l0/psq/", psqt.num_inputs(), width);
        l0.init_with_effective_input_size(32);
        let l0f = builder.new_weights("l0/fac", (width, 768), InitSettings::Zeroed);
        let l0w = l0.weights + l0f.repeat(psqt.num_inputs() / 768);
        ...
        // Feature transformer half -> crelu; pairwise product of halves.
        let ft = |psqt_in, start, end| (l0w.slice_rows(start, end).matmul(psqt_in) + l0.bias.slice_rows(start, end)).crelu();
        let stm_hidden = ft(stm_psqt, 0, width / 2) * ft(stm_psqt, width / 2, width);
        let ntm_hidden = ft(ntm_psqt, 0, width / 2) * ft(ntm_psqt, width / 2, width);
    ...
    optimiser.set_params_for_weight("l0/psq/w", l0_clip);
    ...
    // Saved layout == engine/src/nnue/v3.rs module w1024nt (HAS_THREATS = false): psq i16 | ft bias i16 |
    // l1 w i8 (transposed) | l1 b f32 | l2 w f32 (transposed) | l2 b f32 | l3 w f32 (transposed) | l3 b f32.
    let saved_format = vec![
        SavedFormat::id("l0/psq/w")
            ...
            .round()
            .quantise::<i16>(Q0),
        SavedFormat::id("l0/psq/b").round().quantise::<i16>(Q0),
    ...
    println!("anna-v4 trainer (no threat inputs) | net {} | data {:?} | stages {}/{}/{} SB | batch {} x {} | width {} L2 {} L3 {}", net_id, paths, sb0, sb1, sb2, batch_size, batches_per_sb, width, L2, L3);
    let reader = SfBinpackLoader::new_concat_multiple(&paths, buffer_mb, loader_threads, filter);
    let params_tuple = (&inputs, psqt, output_buckets);
    ...
    run(
        ...
        .boxed(),
        make_psq_inputs_mapper(params_tuple, wdl::ConstantWDL { value: 0.2 }),
    );
    // Stage 1: main run.
    run(
        1,
        ...
        make_psq_inputs_mapper(params_tuple, wdl::LinearWDL { start: 0.2, end: 0.5 }),
    );
    ...
    run(
        ...
        make_psq_inputs_mapper(params_tuple, wdl::ConstantWDL { value: 1.0 }),
    );
    ...
    let evaluator_mapper = make_psq_inputs_mapper(params_tuple, wdl::ConstantWDL { value: 0.0 });
    for fen in [
