...
while true; do
  newest=$(timeout 40 ssh -p "$PORT" -o StrictHostKeyChecking=no -o ConnectTimeout=20 root@"$HOST" 'ls -dt /workspace/checkpoints/anna-v6-*/ 2>/dev/null | head -1' 2>/dev/null | grep -vE "^Welcome|^Have fun" | sed 's#/*$##' | xargs -r basename)
