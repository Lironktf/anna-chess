...
print('key id', kid, 'len', len(key))"
        ...
        # The arming ssh can hang after the detached process starts (seen 2026-09-13): bound it, then verify separately.
        timeout 40 ssh -p "$PORT" "${SSH_OPTS[@]}" root@"$HOST" "chmod +x /workspace/vast_selfdestruct.sh; grep -q '^.* armed:' /workspace/selfdestruct.log 2>/dev/null && ps -eo args | grep -q '^bash /workspace/vast_selfdestruct.sh' || setsid nohup /workspace/vast_selfdestruct.sh $INSTANCE_ID $(cat "$KEYFILE") $MAX_HOURS 20 /workspace/runs/$RUN/train.log >/dev/null 2>&1 </dev/null & sleep 3; exit 0" || true
        timeout 40 ssh -p "$PORT" "${SSH_OPTS[@]}" root@"$HOST" "ps -eo args | grep -q '^bash /workspace/vast_selfdestruct.sh' && grep -q 'armed:' /workspace/selfdestruct.log && { echo 'self-destruct ARMED:'; tail -n 1 /workspace/selfdestruct.log; } || { echo 'self-destruct NOT running'; exit 5; }"
        echo "$(date -u +%FT%TZ) run start self-destruct armed max_hours=$MAX_HOURS cap=$CAP" >> "$ROOT/runs/$RUN/events.log"
        timeout 300 ssh -p "$PORT" "${SSH_OPTS[@]}" root@"$HOST" "command -v tmux >/dev/null || (apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get install -y -qq tmux >/dev/null); chmod +x /workspace/chess/scripts/cpu_eval_remote.sh; tmux new-session -d -s eval 'RUN_NAME=$RUN CONC=${CONC:-28} GAMES_FAST=${GAMES_FAST:-400} GAMES_LONG=${GAMES_LONG:-100} MATCHES=${MATCHES:-default} CONC_SLOW4=${CONC_SLOW4:-} SLOW4_GAMES=\"${SLOW4_GAMES:-}\" bash /workspace/chess/scripts/cpu_eval_remote.sh'"
