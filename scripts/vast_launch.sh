...
for o in json.load(sys.stdin)[:12]:
    ...
    create)
        ...
        for q in "$ROOT/runs/$RUN/checkpoints"/smoke-*/quantised.bin; do "$ROOT/target/release/engine" netcheck "$q"; done
    ...
    train)
        ...
        ssh -p "$PORT" -o StrictHostKeyChecking=no root@"$HOST" "tmux new-session -d -s train 'bash /workspace/chess/scripts/vast_train_remote.sh $RUN $SB'"
