...
pub mod avx2 {
    ...
    // SAFETY (all functions): pointers come from `[i16; L1]` arrays aligned to 64 bytes (Align64)
    // or from slices of the same length; every load/store is within bounds because L1 is a
    // multiple of CHUNK and we iterate exactly L1/CHUNK times. avx2 is a compile-time target feature.
    ...
    pub fn add_feature(acc: &mut [i16; L1], w: &[i16; L1]) {
        unsafe {
            ...
            for i in 0..L1 / CHUNK {
                _mm256_store_si256(a.add(i), _mm256_add_epi16(_mm256_load_si256(a.add(i)), _mm256_load_si256(b.add(i))));
            }
        }
    }
    #[inline]
    pub fn sub_feature(acc: &mut [i16; L1], w: &[i16; L1]) {
        unsafe {
            ...
            let b = w.as_ptr() as *const __m256i;
            for i in 0..L1 / CHUNK {
                _mm256_store_si256(a.add(i), _mm256_sub_epi16(_mm256_load_si256(a.add(i)), _mm256_load_si256(b.add(i))));
            }
        }
    }
    #[inline]
    pub fn add_sub(out: &mut [i16; L1], prev: &[i16; L1], a: &[i16; L1], s: &[i16; L1]) {
        unsafe {
            ...
            for i in 0..L1 / CHUNK {
                let v = _mm256_sub_epi16(_mm256_add_epi16(_mm256_load_si256(p.add(i)), _mm256_load_si256(a.add(i))), _mm256_load_si256(s.add(i)));
                _mm256_store_si256(o.add(i), v);
            }
        }
    }
    #[inline]
    pub fn add_sub_sub(out: &mut [i16; L1], prev: &[i16; L1], a: &[i16; L1], s1: &[i16; L1], s2: &[i16; L1]) {
        unsafe {
            ...
            for i in 0..L1 / CHUNK {
                let v = _mm256_add_epi16(_mm256_load_si256(p.add(i)), _mm256_load_si256(a.add(i)));
                let v = _mm256_sub_epi16(v, _mm256_load_si256(s1.add(i)));
                let v = _mm256_sub_epi16(v, _mm256_load_si256(s2.add(i)));
                _mm256_store_si256(o.add(i), v);
            }
        }
    }
    ...
    pub fn add_add_sub_sub(
        ...
        unsafe {
            ...
            let s2 = s2.as_ptr() as *const __m256i;
            for i in 0..L1 / CHUNK {
                let v = _mm256_add_epi16(_mm256_load_si256(p.add(i)), _mm256_load_si256(a1.add(i)));
                let v = _mm256_add_epi16(v, _mm256_load_si256(a2.add(i)));
                let v = _mm256_sub_epi16(v, _mm256_load_si256(s1.add(i)));
                let v = _mm256_sub_epi16(v, _mm256_load_si256(s2.add(i)));
                _mm256_store_si256(o.add(i), v);
            }
    ...
    unsafe fn dot_half(acc: *const __m256i, w: *const __m256i, sum: __m256i) -> __m256i {
        ...
        let mut sum = sum;
        for i in 0..L1 / CHUNK {
            let v = _mm256_min_epi16(_mm256_max_epi16(_mm256_load_si256(acc.add(i)), zero), qa);
            // v*w fits in i16 (255 * 127 < 32768); madd with v gives i32 lanes of v*w*v pairs.
            let p = _mm256_mullo_epi16(v, _mm256_load_si256(w.add(i)));
