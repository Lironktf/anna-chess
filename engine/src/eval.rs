//! Evaluation entry point: NNUE when a network is loaded, otherwise a small material/PST fallback
...
/// Simple tapered piece-square tables (PeSTO-like, compact). Indexed from white's view, a1 = 0.
#[rustfmt::skip]
...
const EG_PST: [[i16; 64]; 6] = [
    [  0,  0,  0,  0,  0,  0,  0,  0,  13,  8,  8, 10, 13,  0,  2, -7,   4,  7, -6,  1,  0, -5, -1, -8,
      13,  9, -3, -7, -7, -8,  3, -1,  32, 24, 13,  5, -2,  4, 17, 17,  94,100, 85, 67, 56, 53, 82, 84,
...
/// Hand-crafted fallback evaluation from the side to move's perspective.
pub fn evaluate_hce(pos: &Position) -> Value {
    let mut mg = 0;
    let mut eg = 0;
    let mut phase = 0;
    for s in bits(pos.occupied()) {
        let p = pos.piece_on(s);
        let pt = p.piece_type().idx();
        let sq_idx = if p.color() == Color::White { s as usize } else { (s ^ 56) as usize };
        let sign = if p.color() == Color::White { 1 } else { -1 };
        mg += sign * (MG_VAL[pt] + MG_PST[pt][sq_idx] as i32);
        eg += sign * (EG_VAL[pt] + EG_PST[pt][sq_idx] as i32);
        phase += PHASE_INC[pt];
    }
    let phase = phase.min(24);
    let v = (mg * phase + eg * (24 - phase)) / 24;
...
pub fn evaluate(pos: &Position, net: Option<&AnyNet>, st: &mut AnyState) -> Value {
    let mut v = match net {
        Some(n) => {
            ...
            if crate::params::MAT_SCALE.get() != 0 {
                // Scale with material on the board: the net's scores shrink as pieces come off
                // (Stockfish-style material scaling; SPRT-tested).
                let material = pos.non_pawn_material(Color::White) + pos.non_pawn_material(Color::Black)
                    + 100 * popcount(pos.pieces(PieceType::Pawn)) as i32;
                raw * (crate::params::MAT_SCALE_BASE.get() + material / crate::params::MAT_SCALE_DIV.get()) / 1024
            } else {
                raw
            }
        }
        None => evaluate_hce(pos),
    };
    // Damp toward zero as the 50-move counter grows.
    v = v * (200 - pos.rule50() as i32) / 200;
